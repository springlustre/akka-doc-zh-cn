...
和其它的Akka模块一样，Akka HTTP也是通过 <a href="https://github.com/typesafehub/config">Typesafe</a> Config 进行配置。

通常来讲，你需要提供一个 application.conf 的文件，包含所有的区别于各个Akka模块的参考配置文件默认值的面向应用的设置。

下面是Akka HTTP 模块的相关默认配置值。
...
其他Akka HTTP模块不通过 Typesafe Config 提供任何配置。
